...
def _get_next_deadline(conference: 'Conference', submission_type: SubmissionType, current_date: date) -> Optional[date]:
    """
    Get the next deadline for a conference and submission type.
    ...
    submission_type : SubmissionType
        Type of submission (ABSTRACT, PAPER, etc.)
    current_date : date
        Current date to find next deadline from
        
    Returns
    -------
    Optional[date]
        Next deadline date, or None if no deadline found
    """
    if submission_type not in conference.deadlines:
        return None
    
    deadline = conference.deadlines[submission_type]
    
    # If deadline is in the past, return None
