"""Tests for the greedy scheduler."""

from datetime import date, timedelta
from typing import Dict, List, Any

import pytest

from core.models import SubmissionType, ConferenceType, Config, Schedule
from schedulers.greedy import GreedyScheduler
from conftest import create_mock_submission, create_mock_conference, create_mock_config, get_recent_deadline, get_test_date, get_paper_deadline, get_abstract_deadline, get_medium_deadline


class TestGreedyScheduler:
    """Test the greedy scheduler functionality."""

    def test_greedy_scheduler_initialization(self, empty_config: Config) -> None:
        """Test greedy scheduler initialization."""
        scheduler: GreedyScheduler = GreedyScheduler(empty_config)
        
        assert scheduler.config == empty_config
        assert hasattr(scheduler, 'schedule')

    def test_schedule_empty_config(self, empty_config: Config) -> None:
        """Test scheduling with empty config."""
        scheduler: GreedyScheduler = GreedyScheduler(empty_config)
        
        result: Schedule = scheduler.schedule()
        assert isinstance(result, Schedule)
        assert len(result) == 0

    def test_schedule_single_submission(self) -> None:
        """Test scheduling a single submission."""
        print("Debug: Starting test_schedule_single_submission")
        submission = create_mock_submission(
            "paper1", "Test Paper", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        print(f"Debug: Created submission: {submission.id}")
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        print(f"Debug: Created conference: {conference.name}")
        config: Config = create_mock_config([submission], [conference])
        print(f"Debug: Created config with {len(config.submissions)} submissions")
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        print(f"Debug: Created scheduler with {len(scheduler.submissions)} submissions")
        print(f"Debug: Test setup - submission: {submission.id}, conference: {conference.name}, deadline: {conference.deadlines}")
        print(f"Debug: Config has {len(config.submissions)} submissions and {len(config.conferences)} conferences")
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result) == 1
        assert "paper1" in result
        assert isinstance(result.intervals["paper1"].start_date, date)

    def test_schedule_with_dependencies(self) -> None:
        """Test scheduling with dependencies."""
        submission1 = create_mock_submission(
            "paper1", "Dependency Paper", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        submission2 = create_mock_submission(
            "paper2", "Dependent Paper", SubmissionType.PAPER, "conf1",
            depends_on=["paper1"], engineering=True
        )
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config([submission1, submission2], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result) >= 1
        assert "paper1" in result
        
        # If both are scheduled, check dependency constraint
        if "paper2" in result:
            # Dependent paper should start after dependency ends
            paper1_end: date = result.intervals["paper1"].end_date + timedelta(days=config.min_paper_lead_time_days)
            assert result.intervals["paper2"].start_date >= paper1_end

    def test_schedule_respects_earliest_start_date(self) -> None:
        """Test that schedule respects earliest start dates."""
        earliest_start: date = get_test_date(15)  # 15 days from now
        submission = create_mock_submission(
            "paper1", "Test Paper", SubmissionType.PAPER, "conf1",
            earliest_start_date=earliest_start, engineering=True
        )
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config([submission], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        if "paper1" in result:
            assert result.intervals["paper1"].start_date >= earliest_start

    def test_schedule_respects_deadlines(self) -> None:
        """Test that schedule respects deadlines."""
        deadline: date = get_paper_deadline()
        submission = create_mock_submission(
            "paper1", "Test Paper", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: deadline}
        )
        config: Config = create_mock_config([submission], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        if "paper1" in result:
            # Start date + duration should be before deadline
            end_date: date = result.intervals["paper1"].start_date + timedelta(days=config.min_paper_lead_time_days)
            assert end_date <= deadline

    def test_schedule_respects_concurrency_limit(self) -> None:
        """Test that schedule respects concurrency limits."""
        submissions: List[Any] = []
        for i in range(5):
            submission = create_mock_submission(
                f"paper{i}", f"Test Paper {i}", SubmissionType.PAPER, "conf1",
                engineering=True
            )
            submissions.append(submission)
        
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config(submissions, [conference], max_concurrent_submissions=2)
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        
        # Check that no more than max_concurrent_submissions are scheduled on the same day
        scheduled_dates: List[date] = [interval.start_date for interval in result.intervals.values()]
        for i, date1 in enumerate(scheduled_dates):
            same_date_count: int = sum(1 for d in scheduled_dates if d == date1)
            assert same_date_count <= config.max_concurrent_submissions

    def test_schedule_with_priority_weights(self) -> None:
        """Test that schedule respects priority weights."""
        # Create submissions with different priorities
        engineering_paper = create_mock_submission(
            "paper1", "Engineering Paper", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        medical_paper = create_mock_submission(
            "paper2", "Medical Paper", SubmissionType.PAPER, "conf1",
            engineering=False
        )
        abstract = create_mock_submission(
            "abstract1", "Abstract", SubmissionType.ABSTRACT, "conf1"
        )
        
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline(),
             SubmissionType.ABSTRACT: get_abstract_deadline()},
            conf_type=ConferenceType.MEDICAL
        )
        
        # Custom priority weights
        priority_weights: Dict[str, float] = {
            "engineering_paper": 3.0,
            "medical_paper": 1.0,
            "abstract": 0.5
        }
        
        config: Config = create_mock_config([engineering_paper, medical_paper, abstract], [conference])
        config.priority_weights = priority_weights
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result) >= 1
        
        # Engineering paper should be scheduled first (highest priority)
        if "paper1" in result and "paper2" in result:
            # Engineering paper should start before or on the same day as medical paper
            assert result.intervals["paper1"].start_date <= result.intervals["paper2"].start_date

    def test_schedule_with_impossible_constraints(self) -> None:
        """Test scheduling with impossible constraints."""
        # Create submission with impossible constraints
        submission = create_mock_submission(
            "paper1", "Test Paper", SubmissionType.PAPER, "conf1",
            earliest_start_date=date(2025, 12, 1), engineering=True  # Start after deadline
        )
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: date(2025, 6, 1)}
        )
        config: Config = create_mock_config([submission], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        # Should return empty schedule or skip impossible submission
        assert isinstance(result, Schedule)

    def test_schedule_with_abstract_and_paper(self) -> None:
        """Test scheduling with abstract and paper submissions."""
        abstract = create_mock_submission(
            "abstract1", "Test Abstract", SubmissionType.ABSTRACT, "conf1"
        )
        paper = create_mock_submission(
            "paper1", "Test Paper", SubmissionType.PAPER, "conf1",
            depends_on=["abstract1"], engineering=True
        )
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.ABSTRACT: date(2025, 10, 1),
             SubmissionType.PAPER: date(2025, 12, 1)},
            conf_type=ConferenceType.MEDICAL
        )
        config: Config = create_mock_config([abstract, paper], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result) >= 1
        
        # If both are scheduled, check dependency constraint
        if "abstract1" in result and "paper1" in result:
            abstract_end: date = result.intervals["abstract1"].start_date + timedelta(days=config.min_abstract_lead_time_days)
            assert result.intervals["paper1"].start_date >= abstract_end

    def test_schedule_with_engineering_and_medical_papers(self) -> None:
        """Test scheduling with engineering and medical papers."""
        engineering_paper = create_mock_submission(
            "paper1", "Engineering Paper", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        medical_paper = create_mock_submission(
            "paper2", "Medical Paper", SubmissionType.PAPER, "conf2",
            engineering=False
        )
        engineering_conference = create_mock_conference(
            "conf1", "Engineering Conference", 
            {SubmissionType.PAPER: get_paper_deadline()},
            conf_type=ConferenceType.ENGINEERING
        )
        medical_conference = create_mock_conference(
            "conf2", "Medical Conference", 
            {SubmissionType.PAPER: get_paper_deadline()},
            conf_type=ConferenceType.MEDICAL
        )
        config: Config = create_mock_config([engineering_paper, medical_paper], [engineering_conference, medical_conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result) >= 1

    def test_schedule_with_conference_assignment(self) -> None:
        """Test scheduling with conference assignment."""
        submission = create_mock_submission(
            "paper1", "Test Paper", SubmissionType.PAPER, None,  # No conference assigned initially
            preferred_conferences=["Test Conference"], engineering=True
        )
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config([submission], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        # Should be able to schedule even with conference assignment

    def test_schedule_with_multiple_conferences(self) -> None:
        """Test scheduling with multiple conferences."""
        submission1 = create_mock_submission(
            "paper1", "Test Paper 1", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        submission2 = create_mock_submission(
            "paper2", "Test Paper 2", SubmissionType.PAPER, "conf2",
            engineering=True
        )
        conference1 = create_mock_conference(
            "conf1", "Test Conference 1", 
            {SubmissionType.PAPER: get_medium_deadline()}
        )
        conference2 = create_mock_conference(
            "conf2", "Test Conference 2", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config([submission1, submission2], [conference1, conference2])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result.intervals) >= 1

    def test_schedule_with_complex_dependencies(self) -> None:
        """Test scheduling with complex dependency chains."""
        # Create a chain: A -> B -> C
        submission_a = create_mock_submission(
            "paper_a", "Paper A", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        submission_b = create_mock_submission(
            "paper_b", "Paper B", SubmissionType.PAPER, "conf1",
            depends_on=["paper_a"], engineering=True
        )
        submission_c = create_mock_submission(
            "paper_c", "Paper C", SubmissionType.PAPER, "conf1",
            depends_on=["paper_b"], engineering=True
        )
        
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config([submission_a, submission_b, submission_c], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result.intervals) >= 1
        
        # If all are scheduled, check dependency chain
        if all(paper in result.intervals for paper in ["paper_a", "paper_b", "paper_c"]):
            # A should start first
            assert result.intervals["paper_a"].start_date <= result.intervals["paper_b"].start_date
            assert result.intervals["paper_b"].start_date <= result.intervals["paper_c"].start_date
            
            # B should start after A ends
            a_end: date = result.intervals["paper_a"].start_date + timedelta(days=config.min_paper_lead_time_days)
            assert result.intervals["paper_b"].start_date >= a_end
            
            # C should start after B ends
            b_end: date = result.intervals["paper_b"].start_date + timedelta(days=config.min_paper_lead_time_days)
            assert result.intervals["paper_c"].start_date >= b_end

    def test_schedule_with_parallel_dependencies(self) -> None:
        """Test scheduling with parallel dependencies."""
        # Create: A -> B, A -> C (B and C both depend on A)
        submission_a = create_mock_submission(
            "paper_a", "Paper A", SubmissionType.PAPER, "conf1",
            engineering=True
        )
        submission_b = create_mock_submission(
            "paper_b", "Paper B", SubmissionType.PAPER, "conf1",
            depends_on=["paper_a"], engineering=True
        )
        submission_c = create_mock_submission(
            "paper_c", "Paper C", SubmissionType.PAPER, "conf1",
            depends_on=["paper_a"], engineering=True
        )
        
        conference = create_mock_conference(
            "conf1", "Test Conference", 
            {SubmissionType.PAPER: get_paper_deadline()}
        )
        config: Config = create_mock_config([submission_a, submission_b, submission_c], [conference])
        
        scheduler: GreedyScheduler = GreedyScheduler(config)
        result: Schedule = scheduler.schedule()
        
        assert isinstance(result, Schedule)
        assert len(result.intervals) >= 1
        
        # If all are scheduled, check dependencies
        if all(paper in result.intervals for paper in ["paper_a", "paper_b", "paper_c"]):
            # A should start first
            assert result.intervals["paper_a"].start_date <= result.intervals["paper_b"].start_date
            assert result.intervals["paper_a"].start_date <= result.intervals["paper_c"].start_date
            
            # B and C should start after A ends
            a_end: date = result.intervals["paper_a"].start_date + timedelta(days=config.min_paper_lead_time_days)
            assert result.intervals["paper_b"].start_date >= a_end
            assert result.intervals["paper_c"].start_date >= a_end
